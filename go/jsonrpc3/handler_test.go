...
func TestIntrospectionIntegration(t *testing.T) {
	...
	handler := NewHandler(session, root, NewNoOpCaller(), mimeTypes)
...
func NewBidirectionalPair(clientRoot, serverRoot Object) *BidirectionalPair {
	...
	clientHandler := NewHandler(clientSession, clientRoot, NewNoOpCaller(), nil)
	serverHandler := NewHandler(serverSession, serverRoot, NewNoOpCaller(), nil)
