...
Objects MAY implement a `$methods` method that returns detailed information about all methods the object supports.

**Parameters:** None

**Returns:** An array of objects, where each object describes a method supported by the object. Each method object has the following structure:
...
- `name` (required): The method name as a string
- `description` (optional): Human-readable description of what the method does
- `params` (optional): Parameter specification - either an object for named parameters or an array for positional parameters
- `category` (optional): A category for organizing methods (e.g., "math", "io", "admin")
...
**Implementation Notes:**
- The `$methods` method itself SHOULD be included in the returned array
- If the object also implements `$type`, it SHOULD be included in the array
- Only the `name` field is required; all other fields (`description`, `params`, `category`) are optional
